...
# Sentinel: pass as `duration` to any add_*_effect / GSAP method to run
# the effect from its start_time until the natural end of the clip.
CLIP_END = "clip_end"
...
class VideoPipeline:
    ...
    def _build_effects_from_props(
        self, from_props: dict, to_props: dict, easing
    ) -> list:
        ...
        if blur_keys & (from_props.keys() | to_props.keys()):
            ...
            effects.append(
                BlurEffect(start_blur=start_blur, end_blur=end_blur, easing=easing)
            )
        ...
        if shift_keys & (from_props.keys() | to_props.keys()):
            ...
            angle = to_props.get(
                "rgb_shift_angle", from_props.get("rgb_shift_angle", 0.0)
            )
            effects.append(
                RGBShiftEffect(
        ...
        if "zoom" in (from_props.keys() | to_props.keys()):
            ...
            effects.append(
                ZoomEffect(start_zoom=start_zoom, end_zoom=end_zoom, easing=easing)
            )
        ...
        if color_keys & (from_props.keys() | to_props.keys()):
            effects.append(
                ColorAdjustEffect(
                    start_params={
                        k: from_props[k] for k in color_keys if k in from_props
                    },
    ...
    def fromTo(
        ...
        Example:
            ...
            pipeline.fromTo(clip_idx=0, duration=CLIP_END,
    ...
    def render(self, output_path: str):
        ...
        while current_clip_idx < len(self.clips):
            ...
            if AudioSegment is not None:
                ...
                if final_audio is None:
                    ...
                    prev_trans = (
                        ...
                        if current_clip_idx - 1 < len(self.transitions)
                        else None
                    )
                    ...
                    if prev_trans_dur > 0:
                        ...
                        fade_ms = min(
                            fade_ms, len(final_audio) - 1, len(audio_clip) - 1
                        )
                        if fade_ms > 0:
                            final_audio = final_audio.append(
                                audio_clip, crossfade=fade_ms
                            )
                        else:
                            final_audio += audio_clip
                    else:
            ...
            has_transition = current_clip_idx < len(
                self.transitions
            ) and current_clip_idx + 1 < len(self.clips)
            ...
            def apply_local_effects(frame, clip_dict, local_time):
                ...
                for eff in clip_dict["effects"]:
                    ...
                    eff_dur = (
                        eff["duration"]
                        if eff["duration"] > 0
                        else max(0.001, c_dur - eff_start)
                    )
            ...
            for _ in range(frames_to_read):
                ...
                frame = apply_local_effects(
                    frame,
                    self.clips[current_clip_idx],
                    clip_local_times[current_clip_idx],
                )
            ...
            if has_transition and trans_frames > 0:
                ...
                for t_f in range(trans_frames):
                    ...
                    frame1 = apply_local_effects(
                        frame1,
                        self.clips[current_clip_idx],
                        clip_local_times[current_clip_idx],
                    )
                    frame2 = apply_local_effects(
                        frame2,
                        self.clips[current_clip_idx + 1],
                        clip_local_times[current_clip_idx + 1],
                    )
...
if __name__ == "__main__":
    ...
    pipeline.add_transition(
        ZoomTransition(duration=0.3, easing=(0.75, 0, 0.25, 1), mode="inout")
    )
    ...
    pipeline.add_clip_effect(
        clip_idx=0, effect=title_text, start_time=1.0, duration=CLIP_END
    )
    ...
    pipeline.from_(
        clip_idx=1, duration=0.8, brightness=-120, saturation=0.0, easing="ease_out"
    )
    pipeline.to(
        clip_idx=1,
        duration=1.5,
