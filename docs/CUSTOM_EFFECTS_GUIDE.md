...
| `zoom_to_point` | `dict` or `tuple` | — | TikTok-style focus-pull. Pass a dict with `center`, `start_zoom`, `end_zoom`, or a tuple `(cx, cy)`. See examples below. |
...
**Zoom to a point (TikTok style):**
```python
# Focus pull to 30% from left, 70% from top over 1.5s
pipeline.to(clip_idx=0, duration=1.5,
            zoom_to_point={"center": (0.3, 0.7), "start_zoom": 1.0, "end_zoom": 2.0},
            easing="ease_in_out")

# Animated center using a callable (drifts over time)
pipeline.to(clip_idx=1, duration=3.0,
            zoom_to_point={"center": lambda t: (0.5 + 0.2*math.sin(t), 0.5), "end_zoom": 1.8})

# Shortcut: pass just a normalized tuple, uses default zoom values
pipeline.fromTo(clip_idx=0, duration=2.0,
                from_props={"zoom_to_point": (0.2, 0.5)},
                to_props={"zoom_to_point": (0.8, 0.5)})
```
